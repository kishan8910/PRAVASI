...
{ 
	...
	if(!$input_errors)
	{
		$sql = "SELECT email , random_number, aadhar_no FROM forgot_password WHERE aadhar_no=\"$aadhar_no\" and random_number = \"$rand_number\"";
		$result = sql_query($sql, $connect);
				
		if(sql_num_rows($result) )
		{
		
			$sql = "SELECT id from user where aadhar_no=\"$aadhar_no\"";
			$result1 = sql_query($sql, $connect);
			if(sql_num_rows($result1) )
			{
				$row = sql_fetch_array($result1);		
				$studID = $row[0];
				$sql = "UPDATE user SET  password =\"".md5($reset_password)."\"  where id =\"$studID\"";
