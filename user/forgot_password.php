...
include "../libcommon/phpmailer/class.phpmailer.php";
			...
			if(isset($_POST['upload'])) 
			{
				...
				$aadhar_no =  sql_real_escape_string($_POST['aadhar_no']);
				...
				if(!$input_errors)
				{
					...
					if(sql_num_rows($result) )
					{
						...
						$sql = "insert into forgot_password(email,random_number,aadhar_no) values('".$email."','".$encrypt."','".$aadhar_no."')";
						$result = sql_query($sql, $connect);
					
						$MailSubject = "Password Recovery Mail";
                		$MailBody = "Dear Candidate, <br><br>Click here to reset your password ".$website_url."student/pswd_reset.php?email=".$email."&key=".$encrypt."</a><br/> <br/>";  
               			
               			// $response = sendEmail($MailSubject, $MailBody, $email);
               			$response = sendEmail($MailBody,0,$email);
               			if($response == 1)
               			{
...
<div class="col s10 offset-s2">
		...
		<div class="input-field col s5 validation">
		    <i class="material-icons prefix">mail</i>
		    <input required type='number' name="aadhar_no" placeholder="Aadhar number" value="<?php echo $aadhar_no; ?>"autocomplete="off"/>
...
function sendEmail($msgBody,$addAddressFlag,$to)
	{
		$mail = new PHPMailer();
		...
		$mail->Username = "dancestudioweb@gmail.com";  // SMTP username
		$mail->Password = "kish123)(*!@#098"; // SMTP password
		$mail->Port = 465;
		...
		if ($addAddressFlag == 1) 
		{
			...
			$mail->AddAddress('kishansureshin@gmail.com', '');
		}
		else
		{
			$mail->AddAddress($to, "");
